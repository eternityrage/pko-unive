...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "🦜 Picko's Daily Chaos!\n\n#parrot #funnyparrot #pickouniverse #greenparrot"
        
        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "🦜 New video from Picko Universe!"
        ...
        video = upload.video(
            ...
            name=title or 'Picko Universe Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Picko's Daily Chaos! 🦜"
    title = sys.argv[3] if len(sys.argv) > 3 else "Picko Universe"
